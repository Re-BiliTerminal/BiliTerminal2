    ...
    public static TypeAdapter<?> getTypeAdapter(MainConstructor mainConstructor,
        ...
        if (TypeAdapter.class.isAssignableFrom(value)) {
            ...
            if (JsonSerializer.class.isAssignableFrom(value)) {
                ...
                serializer = mainConstructor.get(gson, TypeToken.get(serializerClass)).construct();
            }
            
            if (JsonDeserializer.class.isAssignableFrom(value)) {
                Class<JsonDeserializer<?>> deserializerClass = (Class<JsonDeserializer<?>>) value;
                deserializer = mainConstructor.get(gson, TypeToken.get(deserializerClass)).construct();
            }
            
            @SuppressWarnings("unchecked")
            TreeTypeAdapter<?> treeTypeAdapter = new TreeTypeAdapter(serializer, deserializer, gson, fieldType, null);
