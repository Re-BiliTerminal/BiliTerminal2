...
package com.huanli233.bilizepam.ui.widget.wearable;
...
import com.huanli233.bilizepam.utils.SystemConfigurationKt;
...
public class WearableRecyclerView extends RecyclerView {
    ...
    public WearableRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle,
        ...
        if (attrs != null) {
            ...
            mTopPaddingEnabled = a.getBoolean(R.styleable.WearableRecyclerView_topPadding, false);
            mBottomPaddingEnabled = a.getBoolean(R.styleable.WearableRecyclerView_bottomPadding, true);
    ...
    void setupCenteredPadding() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M || !getResources().getConfiguration().isScreenRound() || getChildCount() < 1) {
            return;
        }
        ...
        if ((mTopPaddingEnabled && getPaddingTop() != desiredPadding) ||
                (mBottomPaddingEnabled && getPaddingBottom() != desiredPadding)) {
            ...
            // can be the same.
            setPadding(getPaddingLeft(), mTopPaddingEnabled ? desiredPadding : mOriginalPaddingTop, getPaddingRight(), mBottomPaddingEnabled ? desiredPadding : mOriginalPaddingBottom);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (mCircularScrollingEnabled && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1 && mScrollManager.onTouchEvent(event)) {
            return true;
        }
