...
/**
 * A page indicator for {@link ViewPager2} which identifies the current page in relation to
...
public class PageIndicatorView extends View {
    ...
    public PageIndicatorView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDotFadeOutDelay =
                a.getInt(R.styleable.PageIndicatorView_wsPageIndicatorDotFadeOutDelay, 3000);
        mDotFadeOutDuration =
        ...
        mDotFadeWhenIdle =
                a.getBoolean(R.styleable.PageIndicatorView_wsPageIndicatorDotFadeWhenIdle, true);
        ...
        mCurrentViewPagerState = ViewPager2.SCROLL_STATE_IDLE;
        if (isInEditMode()) {
    ...
    /**
     * Supplies the ViewPager instance, and attaches this views {@link ViewPager2.OnPageChangeCallback} to the
     * pager.
     *
    ...
    public void setPager(ViewPager2 pager) {
        pager.registerOnPageChangeCallback(new ViewPager2.OnPageChangeCallback() {
            ...
            public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                PageIndicatorView.this.onPageScrolled(position, positionOffset, positionOffsetPixels);
            }

            @Override
            public void onPageScrollStateChanged(int state) {
                PageIndicatorView.this.onPageScrollStateChanged(state);
            }
        });
        ...
        if (adapter != null) {
            adapter.registerAdapterDataObserver(new RecyclerView.AdapterDataObserver() {
                ...
                public void onChanged() {
                    ...
                    updateNumberOfPositions(itemCount);
                    if (mSelectedPosition >= itemCount && itemCount > 0) {
                        onPageSelected(Math.min(mSelectedPosition, itemCount - 1));
                    } else if (itemCount == 0) {
                        onPageSelected(0);
                    }
                    if (pager.getCurrentItem() >= itemCount && itemCount > 0) {
                        pager.setCurrentItem(Math.min(pager.getCurrentItem(), itemCount - 1));
                    } else if (itemCount == 0 && pager.getCurrentItem() != 0) {
                ...
                public void onItemRangeChanged(int positionStart, int itemCount) {
                    int allItemCount = adapter.getItemCount();
                    updateNumberOfPositions(allItemCount);
                    if (mSelectedPosition >= allItemCount && allItemCount > 0) {
                        onPageSelected(Math.min(mSelectedPosition, allItemCount - 1));
                    } else if (allItemCount == 0) {
                        onPageSelected(0);
                    }
                }
                ...
                public void onItemRangeRemoved(int positionStart, int itemCount) {
                    ...
                    if (mSelectedPosition >= allItemCount && allItemCount > 0) {
                        onPageSelected(Math.min(mSelectedPosition, allItemCount - 1));
                        pager.setCurrentItem(mSelectedPosition);
                    } else if (allItemCount == 0) {
                        onPageSelected(0);
                        pager.setCurrentItem(0);
                    }
            ...
            updateNumberOfPositions(adapter.getItemCount());
        }
    ...
    private void updateNumberOfPositions(int count) {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        if (mDotFadeWhenIdle) {
            if (mCurrentViewPagerState == ViewPager2.SCROLL_STATE_DRAGGING) {
                if (positionOffset != 0) {
                    if (!mVisible) {
                        fadeIn();
                    }
