...
class QRLoginFragment : Fragment() {
    ...
    private fun refreshQrCode() {
        lifecycleScope.launch {
            try {
                ThreadManager.runOnUiThread { scanStat.text = "正在获取二维码" }
                ...
                override fun run() {
                    lifecycleScope.launch {
                        try {
                            ...
                            if (isAdded) ThreadManager.runOnUiThread {
                                qrImageView.isEnabled = true
                                scanStat.text = """
                                    无法获取二维码信息，点击上方重试
                                    ${e.message}
    ...
    private suspend fun processLogin(response: ApiResponse<QrCode.LoginResult>) {
        val cookies =
            Preferences.getString(Preferences.COOKIES, "")
